<script lang="ts">
  ...
  import TableOfContents from '../../guides/[slug]/TableOfContents.svelte';
  import { urlFor } from '$lib/utils/sanity/sanityImage';
  import { Link } from 'lucide-svelte';

  let { data: article } = $props();
...
<main class="news-detail-container">
  <article class="article-content">
    <h1>{article.title}</h1>
    <div class="meta">
      <span>By: <span class="author">{article.author}</span></span>
      <span class="category">{article.category}</span>
      <span class="date">Published: {formatDate(article.publishedAt)}</span>
      <span class="date">Updated: {formatDate(article._updatedAt)}</span>
    </div>
    <p class="summary">{article.summary}</p>

    {#if article.featuredImage?.asset}
